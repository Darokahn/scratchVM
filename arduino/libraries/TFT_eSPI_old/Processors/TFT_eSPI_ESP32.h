...
////////////////////////////////////////////////////////////////////////////////////////
// Helper: selects the correct 32-bit mask and register field at compile time
// via constant expressions the compiler/optimizer will fold — no #if on TFT_DC value.

#ifndef TFT_DC
  #define DC_C  // No pin assigned
  #define DC_D  // No pin assigned
#else
  // Standard SPI path
  #define DC_C                                                                \
    (TFT_DC < 32                                                              \
      ? (GPIO.out_w1tc     = (1u << TFT_DC))                                 \
      : (GPIO.out1_w1tc.val = (1u << (TFT_DC - 32))))
  #define DC_D                                                                \
    (TFT_DC < 32                                                              \
      ? (GPIO.out_w1ts     = (1u << TFT_DC))                                 \
      : (GPIO.out1_w1ts.val = (1u << (TFT_DC - 32))))
#endif
...
////////////////////////////////////////////////////////////////////////////////////////
  ...
  #define CS_L       // No pin assigned
  #define CS_H       // No pin assigned

#elif defined(TFT_PARALLEL_8_BIT)
  // Parallel mode: straightforward low/high, no slow-pulse variants
  #define CS_L \
    (TFT_CS < 32 \
      ? (GPIO.out_w1tc     = (1u << TFT_CS)) \
      : (GPIO.out1_w1tc.val = (1u << (TFT_CS - 32))))
  #define CS_H \
    (TFT_CS < 32 \
      ? (GPIO.out_w1ts     = (1u << TFT_CS)) \
      : (GPIO.out1_w1ts.val = (1u << (TFT_CS - 32))))
#else
  // Standard SPI path.
  #define CS_L \
    (TFT_CS < 32 \
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    #ifndef TFT_MISO_DEF
      #define TFT_MISO_DEF -1
    #endif

    #ifndef TFT_MOSI_DEF
      #define TFT_MOSI_DEF 13
    #endif
    #if (TFT_MOSI_DEF == -1)
      #undef TFT_MOSI_DEF
      #define TFT_MOSI_DEF 13
      ...
      #if (TFT_MISO_DEF == -1)
        #undef TFT_MISO_DEF
        #define TFT_MISO_DEF TFT_MOSI_DEF
      #endif
    #endif
